...
/**
 * Displays a XML view for a list of markers
 * @author Roland
 *
 */
public class MarkerTypesXsltView extends AbstractMarkerXsltView  {
	...
	protected Source createXsltSource(Map model, String root,
		...
		List<MarkerType> markers = (List<MarkerType>)model.get("markertypes");
		
		
		Element markersElm = new Element(MARKERTYPES_ELM);
		for (Iterator<MarkerType> iter = markers.iterator(); iter.hasNext();) {
			MarkerType marker = iter.next();
			Element markerElm = createMarkerTypeElement(marker);
			markersElm.addContent(markerElm);
		}
		
		return createDocSource(markersElm);
	}
