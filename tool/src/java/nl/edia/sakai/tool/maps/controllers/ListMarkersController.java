 ...
 *
 */
public class ListMarkersController extends MarkerController {
	...
	public Map<String, Object> processRequest(HttpServletRequest request) throws MapsProtocolException, PermissionException {
		...
		if (request.getParameter(TYPE_ID_ATTR) != null) {
			MarkerType markerType = markerService.readMarkerType(new Long(request.getParameter(TYPE_ID_ATTR)));
			markers = markerService.listMarkersByType(markerType);
		}
		else {
			markers = markerService.listMarkers();
		}

		return createModel("markers", markers);
	}
