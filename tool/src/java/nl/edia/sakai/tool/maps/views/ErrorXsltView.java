...
/**
 * Generates an error XML message.
 * @author Roland
 *
 */
public class ErrorXsltView extends AbstractMarkerXsltView {
	...
	protected Source createXsltSource(Map model, String root, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Element element = new Element(ERROR_ELM);;
		if (exception instanceof MapsProtocolException) {
			MapsProtocolException mpe = (MapsProtocolException)exception;
			element.setAttribute(TYPE_ID_ATTR, mpe.getType());
			element.setAttribute(MESSAGE_ATTR, mpe.getMessage());
		} else if (exception instanceof  PermissionException) {
			element.setAttribute(TYPE_ID_ATTR, "Permission denied");
			element.setAttribute(MESSAGE_ATTR,
					"You do not have permission to call this function.");
		} else if (exception != null){
			element.setAttribute(TYPE_ID_ATTR, "unexpected");
			element.setAttribute(MESSAGE_ATTR,
					exception.getMessage());
		}
		return createDocSource(element);
