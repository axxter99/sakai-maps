 ...
 *
 */
public class MarkersXsltView extends AbstractMarkerXsltView  {
	...
	protected Source createXsltSource(Map model, String root, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Element markersElm = new Element(MARKERS_ELM);
		markersElm.setAttribute(CANCREATE_ATTR, Boolean.toString(SakaiUtils.hasPermission(Permissions.PERMISSION_EDIA_SAKAI_MAPS_MARKER_CREATE)));
		for (Iterator<Marker> iter = markers.iterator(); iter.hasNext();) {
			Marker marker = (Marker) iter.next();
			Element markerElm = createMarkerElement(marker);
